...
export const CreateNewBook = async (req, res, next) => {
  try {
    ...
    //   if (req.user.isAdmin) {
        ...
        cloudinary.uploader.upload(req.file.path, { resource_type: "auto" } ,async function (err, result) {
          if (err) {
            console.log(err);
  ...
  try {
    ...
    //   if (req.user.isAdmin) {
      if(req.files.length > 0){
        ...
        cloudinary.uploader.upload(req.files[0].path,{ resource_type: "auto" }, async function (err, result) {
          if (err) {
            console.log(err);
            return res.status(500).json({
              success: false,
